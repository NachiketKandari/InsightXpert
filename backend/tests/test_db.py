...
def test_connect_and_get_tables(sqlite_db):
...
def test_execute_select(sqlite_db):
    db = DatabaseConnector()
    db.connect(sqlite_db)
...
def test_execute_aggregate(sqlite_db):
    db = DatabaseConnector()
    db.connect(sqlite_db)
...
def test_execute_row_limit(sqlite_db):
    db = DatabaseConnector()
    db.connect(sqlite_db)
...
def test_get_schema_ddl(sqlite_db):
...
def test_get_table_info(sqlite_db):
    engine = create_engine(sqlite_db)
