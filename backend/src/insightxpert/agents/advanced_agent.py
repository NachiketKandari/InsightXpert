"""Advanced analytics agent — enriches analyst SQL results with quantitative analysis.

Runs instead of the statistician when agent_mode == "advanced". Provides a richer
toolkit: time-series analysis, fraud risk scoring, anomaly detection, bank-pair risk
matrices, concentration indices, percentile ranking, and Benford's law validation.
"""
...
logger = logging.getLogger("insightxpert.advanced_agent")
...
async def advanced_analytics_loop(
    question: str,
    ...
    """Run the advanced analytics agent loop on analyst results."""
    cid = conversation_id
    loop_start = time.time()

    if tool_registry is None:
        timeout = config.python_exec_timeout_seconds
        tool_registry = advanced_registry(python_exec_timeout=timeout)
    ...
    logger.info("=" * 60)
    logger.info("ADVANCED AGENT [%s]: processing %d analyst rows", cid, len(analyst_results))
    logger.info("=" * 60)

    yield ChatChunk(
        type="status",
        content="Running advanced analytics...",
        data={"agent": "advanced"},
        conversation_id=cid,
        timestamp=time.time(),
    )
    ...
    for iteration in range(max_iter):
        logger.info("--- Advanced agent iteration %d/%d ---", iteration + 1, max_iter)

        llm_start = time.time()
        try:
            response = await llm.chat(messages, tools=tool_registry.get_schemas())
        except Exception as exc:
            logger.error("Advanced agent LLM call failed: %s", exc, exc_info=True)
            yield ChatChunk(
                type="error",
                content=f"Advanced analytics failed: {exc}",
                data={"agent": "advanced"},
                conversation_id=cid,
                timestamp=time.time(),
            )
        ...
        if response.tool_calls:
            ...
            logger.info("Advanced agent LLM (%.0fms): tool_calls=%s", llm_ms, tool_names)
            ...
            for tc in response.tool_calls:
                yield ChatChunk(
                    type="tool_call",
                    content=f"[Advanced] Calling {tc.name}...",
                    tool_name=tc.name,
                    args=tc.arguments,
                    data={"agent": "advanced"},
                    conversation_id=cid,
                ...
                logger.info("Advanced tool %s (%.0fms): %s", tc.name, tool_ms, result[:200])
                ...
                yield ChatChunk(
                    type="tool_result",
                    data={"agent": "advanced", "tool": tc.name, "result": result},
                    tool_name=tc.name,
                    conversation_id=cid,
                    timestamp=time.time(),
                )
            ...
            logger.info(
                "ADVANCED AGENT DONE [%s] total=%.0fms iterations=%d",
        ...
        total_ms = (time.time() - loop_start) * 1000
        logger.warning(
            "ADVANCED AGENT EXHAUSTED [%s] max iterations=%d total=%.0fms",
            cid, max_iter, total_ms,
        )
        yield ChatChunk(
            type="error",
            content=f"Advanced analytics reached maximum iterations ({max_iter}).",
            data={"agent": "advanced"},
            conversation_id=cid,
            timestamp=time.time(),
        )
